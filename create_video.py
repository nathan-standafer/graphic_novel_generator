...
def generate_ffmpeg_input_file(srt_path, num_images, output_path):
    ...
    for i, block in enumerate(srt_blocks):
        ...
        image_durations.append((f"file 'generated_illustrations/{chapter_dir}/{chapter_dir}_scene_{image_counter:03d}.png'", duration))
        image_counter += 1
...
if __name__ == '__main__':
    ...
    output_video_file = f'{chapter_dir}.mp4'
    audio_file = f'generated_audio/{chapter_dir}.wav'
    srt_file_for_subtitles = f'generated_audio/{chapter_dir}.srt'
...
#    )
    ...
    ffmpeg_command = (
        f"ffmpeg -y -f concat -safe 0 -i {ffmpeg_input_file} -i {audio_file} "
        f"-c:v mpeg4 -b:v 5000k -qscale:v 2 "
        f"-c:a aac -b:a 192k "
